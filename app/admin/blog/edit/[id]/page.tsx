  ...
  useEffect(() => {
    ...
    async function fetchPost() {
      ...
      const { data, error } = await supabase
        .from("posts")
        .select("title, excerpt, content, category, read_time, gradient, date")
        .eq("id", id)
        .single();
    ...
    <div className="space-y-6 text-bone pb-10">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-semibold text-bone">Edit Artikel</h1>
            <p className="text-sm text-mist mt-0.5">
              Perbarui tulisan dengan editor kaya bergaya Medium.
            </p>
          </div>
        </div>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20 text-mist gap-3 bg-coal-900 border border-coal-800 rounded-2xl">
          <Icon icon="solar:refresh-bold" className="text-2xl animate-spin" />
          <span className="text-sm">Memuat isi artikel...</span>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-8 space-y-4">
            ...
            <div className="bg-coal-900 border border-coal-800 rounded-2xl p-6">
              ...
              {/* Only render editor when content is loaded (avoids empty initial state) */}
              {!loading && (
                <RichTextEditor
                  content={content}
                  onChange={setContent}
